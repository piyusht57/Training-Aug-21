...
SELECT CONCAT(e.FirstName,' ', e.LastName) AS 'Name', e.Salary, e.JobId FROM Employees e
...
SELECT CONCAT(FirstName,' ',LastName) AS 'Employee_Name', Salary FROM Employees WHERE DepartmentID IN 
...
SELECT CONCAT(FirstName,' ',LastName) AS 'Employee_Name', Salary FROM Employees WHERE Salary>(SELECT Salary FROM Employees WHERE LastName = 'Bell')
...
SELECT CONCAT(FirstName,' ', LastName) AS 'Name', Salary, DepartmentID FROM Employees WHERE Salary IN (SELECT MIN(Salary) FROM Employees GROUP BY DepartmentID)
...
SELECT CONCAT(FirstName,' ',LastName) AS 'Employee_Name', Salary AS 'More_Than_Average' FROM Employees WHERE Salary>ALL
...
SELECT CONCAT(FirstName,' ',LastName) AS 'Employee_Name', Salary AS 'More_Than_SH_Clerk' FROM Employees WHERE Salary >
(SELECT MAX(Salary) FROM Employees WHERE JobId = 'SH_CLERK') ORDER BY Salary ASC


---12. Write a query to find the names (first_name, last_name) of the employees who are not supervisors. 
SELECT CONCAT(FirstName,' ',LastName) AS 'Not_Supervisors' FROM Employees WHERE EmployeeID NOT IN 
(SELECT DISTINCT (ManagerID) FROM Employees WHERE ManagerID != 0)


---13. Write a query to display the employee ID, first name, last names, and department names of all employees. 
SELECT e.EmployeeID, e.FirstName, e.LastName, d.DepartmentName AS 'Department' FROM Employees e LEFT JOIN Departments d ON e.DepartmentID=d.DepartmentID
...
SELECT DISTINCT Salary AS '5thMaxSalary' FROM (SELECT DENSE_RANK() OVER (ORDER BY Salary DESC) AS 'Pagar', Salary FROM Employees) EmployeeTemp WHERE Pagar = 5 


---17. Write a query to find the 4th minimum salary in the employees table. 
SELECT DISTINCT Salary AS '4thMaxSalary' FROM (SELECT DENSE_RANK() OVER (ORDER BY Salary ASC) AS 'Pagar', Salary FROM Employees) EmployeeTemp WHERE Pagar = 4
